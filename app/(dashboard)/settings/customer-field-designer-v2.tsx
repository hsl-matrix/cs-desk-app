...
import type { Brand } from "./brand-management";
...
// Mock brands (would come from brand management in real app)
const mockBrands: Brand[] = [
  { id: "brand-1", name: "메인 브랜드", description: "", code: "MAIN", isActive: true, createdAt: "2024-01-01" },
  { id: "brand-2", name: "서브 브랜드 A", description: "", code: "SUB_A", isActive: true, createdAt: "2024-01-01" },
  { id: "brand-3", name: "서브 브랜드 B", description: "", code: "SUB_B", isActive: false, createdAt: "2024-01-01" },
];

// Initial templates
const initialTemplates: FieldTemplate[] = [
  {
    id: "template-1",
    name: "기본 양식",
    description: "모든 브랜드에서 사용하는 기본 고객정보 양식",
    brandId: null,
    rows: [{ id: "row-1", fields: [] }],
    fieldLibrary: [...DEFAULT_FIELD_LIBRARY],
    createdAt: "2024-01-01",
    updatedAt: "2024-01-01",
  },
];

export function CustomerFieldDesignerV2() {
  ...
  // Form states for template dialog
  ...
  // Designer states from current template
  const [rows, setRows] = useState<DesignerRow[]>(currentTemplate?.rows || [{ id: "row-1", fields: [] }]);
  const [fieldLibrary, setFieldLibrary] = useState<FieldDefinition[]>(currentTemplate?.fieldLibrary || DEFAULT_FIELD_LIBRARY);
  ...
  const templateCounterRef = useRef(2);

  // Update states when template changes
  useEffect(() => {
    if (currentTemplate) {
      setRows(currentTemplate.rows);
      setFieldLibrary(currentTemplate.fieldLibrary);

      // Calculate available fields
      const usedFieldIds = currentTemplate.rows.flatMap(row =>
        row.fields.map(field => field.fieldId)
      );
      const available = currentTemplate.fieldLibrary
        .filter(field => !usedFieldIds.includes(field.id))
        .map(field => field.id);
      setAvailableFieldIds(available);
    }
  }, [currentTemplate]);
  ...
  const saveCurrentTemplate = useCallback(() => {
    setTemplates(prev => prev.map(t =>
      t.id === currentTemplateId
        ? { ...t, rows, fieldLibrary, updatedAt: new Date().toISOString() }
        : t
    ));
  }, [currentTemplateId, rows, fieldLibrary]);

  // Auto-save on changes
  useEffect(() => {
    const timer = setTimeout(saveCurrentTemplate, 500);
    return () => clearTimeout(timer);
  }, [rows, fieldLibrary, saveCurrentTemplate]);
  ...
  const handleOpenTemplateDialog = (template?: FieldTemplate) => {
    if (template) {
      setEditingTemplate(template);
      setTemplateFormData({
        name: template.name,
        description: template.description,
        brandId: template.brandId || "none",
      });
    } else {
  ...
  const handleSaveTemplate = () => {
    if (!templateFormData.name) return;
    ...
    if (editingTemplate) {
      ...
      setTemplates(prev => prev.map(t =>
        t.id === editingTemplate.id
          ? {
              ...
              brandId: templateFormData.brandId === "none" ? null : templateFormData.brandId || null,
      ...
      const newTemplate: FieldTemplate = {
        id: `template-${templateCounterRef.current++}`,
        name: templateFormData.name,
        description: templateFormData.description,
        brandId: templateFormData.brandId === "none" ? null : templateFormData.brandId || null,
        rows: [{ id: "row-1", fields: [] }],
  ...
  const handleDeleteTemplate = (templateId: string) => {
    if (templates.length <= 1) {
      alert("최소 하나의 양식은 필요합니다.");
      return;
    }

    setTemplates(prev => prev.filter(t => t.id !== templateId));
    if (currentTemplateId === templateId) {
      setCurrentTemplateId(templates.find(t => t.id !== templateId)?.id || "");
    }
  };
  ...
  // Existing designer functions (adapted from original)
  useEffect(() => {
    if (!rows.length) {
    ...
    <>
              ...
              <div className="flex items-center gap-4">
                ...
                <Select value={currentTemplateId} onValueChange={setCurrentTemplateId}>
                  ...
                  <SelectContent>
                    {templates.map((template) => {
                      const brand = mockBrands.find(b => b.id === template.brandId);
                      return (
                ...
                <DropdownMenu>
                  ...
                  <DropdownMenuContent align="start">
                    <DropdownMenuLabel>양식 관리</DropdownMenuLabel>
                    <DropdownMenuItem onClick={() => handleOpenTemplateDialog()}>
                      <Plus className="mr-2 h-4 w-4" />
                      새 양식 추가
                    </DropdownMenuItem>
                    <DropdownMenuItem onClick={() => handleOpenTemplateDialog(currentTemplate)}>
            ...
            {currentTemplate && (
              <div className="rounded-lg border bg-muted/40 p-3">
                <div className="flex items-start justify-between">
                  ...
                  {currentTemplate.brandId && (
                    <div className="text-xs bg-background px-2 py-1 rounded">
                      브랜드: {mockBrands.find(b => b.id === currentTemplate.brandId)?.name}
                    </div>
                  )}
                </div>
              </div>
      ...
      <Dialog open={isTemplateDialogOpen} onOpenChange={setIsTemplateDialogOpen}>
        <DialogContent className="sm:max-w-[500px]">
          <DialogHeader>
            <DialogTitle>
              {editingTemplate ? "양식 수정" : "새 양식 추가"}
            </DialogTitle>
            <DialogDescription>
              필드 양식 정보를 입력하세요. 각 양식은 특정 브랜드와 연결될 수 있습니다.
            </DialogDescription>
          </DialogHeader>
          <div className="grid gap-4 py-4">
            <div className="grid gap-2">
              <Label htmlFor="template-name">양식명 *</Label>
              <Input
                id="template-name"
                value={templateFormData.name}
                onChange={(e) => setTemplateFormData({ ...templateFormData, name: e.target.value })}
                placeholder="예: 기본 고객정보"
              />
            </div>
            <div className="grid gap-2">
              ...
              <Textarea
                ...
                placeholder="이 양식에 대한 설명을 입력하세요"
                rows={3}
              />
            </div>
            <div className="grid gap-2">
              <Label htmlFor="template-brand">브랜드 (선택)</Label>
              <Select
                value={templateFormData.brandId}
                onValueChange={(value) => setTemplateFormData({ ...templateFormData, brandId: value })}
              >
